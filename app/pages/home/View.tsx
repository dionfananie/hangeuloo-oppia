...
export default function Home() {
	...
	const {
		...
		closeModal,
		submitSession,
	} = useHomeView(dashboard?.profile ?? null, practiceUnlocks);
		...
		<div className="app-shell">
			<aside className="sidebar">
				...
				<nav className="main-nav" aria-label="Main navigation">
					{nav.map((item) =>
						item.id === "learn" ? (
							<Link key={item.id} className="nav-item" to="/lessons">
								<Icon name={item.icon} />
								<span>{item.label}</span>
							</Link>
						) : item.id === "practice" ? (
							<Link key={item.id} className="nav-item" to="/practice">
								<Icon name={item.icon} />
								<span>{item.label}</span>
							</Link>
						) : (
							<button
								...
								onClick={() => {
									setActive(item.id);
									if (item.id === "interview") setModal("interview");
								}}
							>
								<Icon name={item.icon} />
								<span>{item.label}</span>
								{item.id === "interview" && <span className="new-badge">NEW</span>}
							</button>
						),
					)}
				</nav>
				<div className="sidebar-bottom">
					...
					<form className="profile-form" method="post" action="/api/auth/logout">
						<button className="profile-row" type="submit" title="Sign out">
							...
							<span>
								...
								<small>Level {profile.level} · Sign out</small>
							</span>
			...
			<main className="main" id="top">
				<header className="mobile-header">
					<a className="brand compact" href="#top">
					...
					<form method="post" action="/api/auth/logout">
						<button className="mobile-avatar" type="submit" title="Sign out">
				...
				<div className="page-wrap">
					<section className="welcome-row fade-in">
						<div>
							...
							<h1>
								안녕하세요, {firstName}! <span className="wave">👋</span>
							</h1>
							<p>Ready for another happy step in Korean?</p>
						</div>
						<div className="top-stats">
							...
							<button className="heart-button" aria-label="Review words due">
					...
					{active === "learn" ? (
						<LevelCatalog currentLevel={profile.level} />
					) : active === "practice" ? (
						<PracticeLibrary
							dashboard={dashboard}
							openActivity={openActivity}
							practiceUnlocks={practiceUnlocks}
						/>
					) : active === "progress" ? (
						...
						<>
							<section className="hero-card fade-in delay-1">
								<div className="hero-content">
									<span className="today-pill">
										<Icon name="sparkles" size={15} /> TODAY'S MISSION
									</span>
									<h2>Every word brings you closer.</h2>
									<p>Complete your daily practice and keep your streak shining!</p>
									<div className="mission-progress">
										<div className="progress-label">
											<span>Daily goal</span>
											<strong>
												{Math.min(stats.todayActivities, dailyActivities)} of {dailyActivities} activities
											</strong>
										</div>
										<div className="progress-track">
											<span
												style={{
													width: `${Math.min(100, (stats.todayActivities / dailyActivities) * 100)}%`,
												}}
											/>
										</div>
									</div>
									<div className="hero-actions">
										<button
											...
											onClick={() =>
												suggestedUnlocked
													? openActivity(suggestedPractice)
													: window.location.assign("/lessons")
											}
										>
											<span className="play-circle">
												<Icon name={suggestedUnlocked ? "play" : "book"} size={17} />
											</span>
											{suggestedUnlocked ? "Start today's practice" : "Learn before you practice"}
											<Icon name="arrow" size={18} />
										</button>
										<Link className="lessons-cta" to="/lessons">
											<Icon name="book" size={16} /> Browse lessons
										</Link>
									</div>
								</div>
								<div className="hero-visual" aria-hidden="true">
									...
									<img className="mascot" src="/hangeuloo-icon-white-small.png" alt="" />
									<div className="speech-bubble">
										화이팅!<small>You got this!</small>
									</div>
								</div>
							</section>
							...
							<section className="section-block fade-in delay-2">
								<div className="section-heading">
									...
									<button onClick={() => setActive("practice")}>
										View all <Icon name="arrow" size={15} />
									</button>
								</div>
								<div className="activity-grid">
									{activities.map((item) => {
										const unlocked = practiceUnlocks.includes(practiceTypeById[item.id]);
										return (
											<button
												key={item.id}
												className={`activity-card ${unlocked ? "" : "practice-locked"}`}
												onClick={() =>
													unlocked ? openActivity(item.id) : window.location.assign("/lessons")
												}
											>
												<span className={`activity-icon ${item.color}`}>
													<Icon name={unlocked ? item.icon : "lock"} size={25} />
												</span>
												<span className="activity-text">
													<strong>{item.title}</strong>
													<small>{unlocked ? item.copy : "Finish a lesson to unlock"}</small>
													<span>
														<Icon name={unlocked ? "clock" : "book"} size={13} />
														{unlocked
															? item.id === "review"
																? `${stats.dueCount} words due`
																: item.meta
															: "Learn first"}
													</span>
												</span>
												{unlocked && <span className="card-reward">{item.xp}</span>}
							...
							<div className="lower-grid fade-in delay-3">
									...
									{pathSummary ? (
											...
											<div className="circle-progress">
												<svg viewBox="0 0 42 42">
													...
													<circle
														...
														cy="21"
														r="16"
														style={{ strokeDashoffset: 100 - continueLesson.progress }}
													/>
												</svg>
												<span>{continueLesson.progress}%</span>
											</div>
											<div>
												<strong>{continueLesson.title}</strong>
												<small>
													{continueLesson.itemCount} items · {continueLesson.estimatedMinutes} min
												</small>
											</div>
										...
										<div className="lesson-progress">
											<div>
												...
												<small>Start your first structured lesson.</small>
											</div>
											<Link to="/lessons">
												<Icon name="play" size={15} /> Explore
											</Link>
										</div>
									)}
								</section>
								<section className="week-card">
									<div className="week-top">
										<div>
											<h2>This week</h2>
											<p>{stats.weekdays.filter(Boolean).length} active days</p>
										</div>
										<div className="week-xp">
											<Icon name="bolt" size={15} />
											<strong>{stats.weeklyXp}</strong> XP
										</div>
									</div>
									<div className="weekdays">
										{weekdays.map((item, i) => (
											<div key={i}>
												<span
													className={
														stats.weekdays[i] ? "day done" : i === todayWeekdayIndex ? "day today" : "day"
													}
												>
													{stats.weekdays[i] ? (
									...
									<div className="weekly-note">
										...
										<span>
											<strong>{stats.todayXp} XP today.</strong> Each completed round builds durable progress.
										</span>
									</div>
								</section>
							</div>
							...
							<section className="interview-banner fade-in delay-3">
								...
								<div>
									<span className="section-kicker">AI INTERVIEW PRACTICE</span>
									<h2>Build confidence, one answer at a time.</h2>
									<p>Practice real Korean conversations with friendly, actionable feedback.</p>
								</div>
								<button onClick={() => setModal("interview")}>
									Try AI interview <Icon name="arrow" size={17} />
								</button>
								<span className="banner-hangul" aria-hidden="true">
									말
								</span>
							</section>
						</>
			...
			<nav className="mobile-nav" aria-label="Mobile navigation">
				{nav.slice(0, 5).map((item) =>
					item.id === "learn" ? (
						...
						>
							<Icon name={item.icon} />
							<span>{item.id === "interview" ? "Interview" : item.label}</span>
						</button>
					),
				)}
			</nav>
			...
			{modal && (
				...
				>
					<div className="practice-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
						...
						{modal === "interview" ? (
							<InterviewPanel
								scenarios={dashboard.interviewScenarios}
								recording={recording}
								setRecording={setRecording}
								speak={speak}
								done={() => {
									setToast("Interview saved · +35 XP");
									closeModal();
								}}
							/>
						) : (
							<PracticePanel type={modal} dashboard={dashboard} submit={submitSession} speak={speak} />
						)}
					</div>
				</div>
			)}
