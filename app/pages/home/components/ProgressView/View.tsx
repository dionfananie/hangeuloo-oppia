	...
	return (
		<section className="catalog-view fade-in">
			<div className="view-title">
				<span className="section-kicker">LEARNING PROGRESS</span>
				<h2>Your effort is adding up</h2>
				<p>Progress is saved after every completed practice round.</p>
			</div>
			<div className="progress-cards">
				...
				<article>
					...
					<small>Vocabulary mastered</small>
				</article>
			</div>
			<div className="progress-detail">
				<div>
					...
					<p>
						{stats.practicedCount} of {stats.totalVocabulary} available words practiced
					</p>
					<div className="progress-track">
						<span
							style={{
								width: `${stats.totalVocabulary ? (stats.practicedCount / stats.totalVocabulary) * 100 : 0}%`,
							}}
						/>
					</div>
				</div>
				<div>
					<h3>This week</h3>
					<p>
						{stats.weeklyXp} XP across {stats.weekdays.filter(Boolean).length} active days
					</p>
					<div className="weekdays">
						{weekdays.map((item, index) => (
							<div key={index}>
								<span className={stats.weekdays[index] ? "day done" : "day"}>
									{stats.weekdays[index] && <Icon name="check" size={14} />}
								</span>
