	...
	return (
		<section className="catalog-view fade-in">
			<div className="view-title">
				...
				<h2>Seven steps to confident Korean</h2>
				<p>Levels 0–2 are ready now. Complete four practice tracks at your pace.</p>
			</div>
			<div className="level-grid">
				{levelNames.map((name, level) => (
					<article className={level === currentLevel ? "level-card current" : "level-card"} key={name}>
						...
						<div>
							...
							<div className="track-list">
								{["Vocabulary", "Grammar", "Listening", "Speaking"].map((track) => (
									<span key={track}>
				...
				{future.map((name, index) => (
					<article className="level-card locked" key={name}>
						<span className="level-number">
							<Icon name="lock" size={20} />
						</span>
						<div>
							<small>COMING SOON · LEVEL {index + 3}</small>
							<h3>{name}</h3>
							<p>Preview the next milestone while you build strong foundations.</p>
						</div>
					</article>
				))}
			</div>
