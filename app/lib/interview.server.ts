...
function validateFeedback(value: unknown): InterviewFeedback {
	...
	if (scores.some((score) => !Number.isInteger(score) || Number(score) < 0 || Number(score) > 100))
		throw new Error("AI returned invalid scores");
	if (
		![data.correctedAnswer, data.naturalAnswer, data.nextPractice].every(
			(text) => typeof text === "string" && text.length > 0,
		)
	)
		throw new Error("AI returned incomplete feedback");
	if (
		![data.mistakes, data.recommendedWords].every(
...
export async function generateFeedback(
	ai: Ai,
	input: { scenario: InterviewScenario; answer: string; language: GuideLanguage },
) {
	...
	const output = await ai.run("@cf/meta/llama-3.3-70b-instruct-fp8-fast", {
		messages: [
			{
				role: "system",
				content: `You are Hangeuloo, a kind Korean tutor. Evaluate learner Korean conservatively for CEFR pre-A1 to A2. Explain feedback in ${guidanceLanguage}. Never claim precise pronunciation assessment. Return only the requested JSON.`,
			},
			{
				role: "user",
				content: `Level: ${input.scenario.level}\nQuestion: ${input.scenario.questionKo}\nLearner answer: ${input.answer}\nScore grammar, content relevance, and text-based fluency. Correct errors, provide a natural Korean version, list concise mistakes and useful Korean words, then give one concrete next practice action.`,
			},
		],
...
export async function transcribeKorean(ai: Ai, audio: File) {
	...
	return {
		...
		duration: Math.round(output.transcription_info?.duration ?? 0),
	};
}
...
export async function saveInterview(
	...
	const existing = await db
		.prepare("SELECT id FROM interview_sessions WHERE idempotency_key = ? AND user_id = ?")
		.bind(options.idempotencyKey, options.userId)
		.first<{ id: string }>();
	...
	await db.batch([
		db
			.prepare(
				"INSERT INTO interview_sessions (id, idempotency_key, user_id, scenario_id, answer_mode, duration_seconds) VALUES (?, ?, ?, ?, ?, ?)",
			)
			.bind(
				sessionId,
		...
		db
			.prepare("INSERT INTO interview_turns (id, session_id, question_ko, answer_text) VALUES (?, ?, ?, ?)")
			.bind(crypto.randomUUID(), sessionId, options.scenario.questionKo, options.answer),
		db
			.prepare(
				`INSERT INTO ai_feedback (id, session_id, model, overall_score, fluency_score, grammar_score, content_score, corrected_answer, natural_answer, mistakes, recommended_words, next_practice)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			)
			.bind(
				crypto.randomUUID(),
				...
				options.feedback.overallScore,
				options.feedback.fluencyScore,
				options.feedback.grammarScore,
				options.feedback.contentScore,
				options.feedback.correctedAnswer,
				options.feedback.naturalAnswer,
				JSON.stringify(options.feedback.mistakes),
				JSON.stringify(options.feedback.recommendedWords),
				options.feedback.nextPractice,
			),
		db
			.prepare(
			...
			activities_completed = activities_completed + 1, correct_answers = correct_answers + 1, total_answers = total_answers + 1`,
			)
			.bind(options.userId, new Date().toISOString().slice(0, 10)),
	]);
	return sessionId;
}
